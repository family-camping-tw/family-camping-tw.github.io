...
function cardTemplate(camp) {
  ...
  return `<article class="camp-card">
    ...
    <dl class="quick-facts"><div><dt>車程</dt><dd>${escapeHtml(camp.車程)}</dd></div><div><dt>海拔</dt><dd>${formatAltitude(camp.海拔高度)}</dd></div><div><dt>場地</dt><dd>${escapeHtml(camp.營地材質)}</dd></div><div><dt>每晚營位</dt><dd>${camp.價格 === "不確定" ? "不確定" : `$${escapeHtml(camp.價格)}`}</dd></div></dl>
...
function renderComparison() {
  ...
  const rows = [
    ...
    ["車程", c => c.車程], ["海拔", c => formatAltitude(c.海拔高度)], ["每晚營位價格", c => c.價格 === "不確定" ? "不確定" : `$${c.價格}`], ["營地材質", c => c.營地材質], ["可在車內過夜", c => c.能否車露],
